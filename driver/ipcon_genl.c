...
static int ipcon_filter(struct sock *dsk, struct sk_buff *skb, void *data)
{
	...
	ipcon_dbg("Multicast to %s@%lu.\n",
			nc_refname(ipcon_nc, ipn->nameid),
...
static void ipcon_notify_worker(struct work_struct *work)
{
	...
	do {
		...
		if (!hash_empty(ipn->ipn_group_ht)) {
			hash_for_each(ipn->ipn_group_ht, bkt, igi, igi_hgroup) {
				...
				ipcon_dbg("Group %s.%s@%d removed.\n",
					nc_refname(ipcon_nc, ipn->nameid),
					nc_refname(ipcon_nc, igi->nameid),
					igi->group);
				...
				if (iw_mc) {
					...
					nc_getname(ipcon_nc, igi->nameid,
						ik->group.name);
					nc_getname(ipcon_nc, ipn->nameid,
						ik->group.peer_name);
					queue_work(ipcon_db->mc_wq,
							&iw_mc->work);
				}

				nc_id_put(ipcon_nc, igi->nameid);
				igi_free(igi);
		...
		if (iw_mc) {
			...
			nc_getname(ipcon_nc, ipn->nameid, ik->peer.name);
			queue_work(ipcon_db->mc_wq, &iw_mc->work);
		}
	} while (0);

	if (ipn)
		nc_id_put(ipcon_nc, ipn->nameid);
	ipn_free(ipn);
	iw_free(iw);
	ipcon_dbg("exit\n");
}
		...
		unsigned int group)
{
	...
	do {
		...
		if (iw) {
			...
			nc_getname(ipcon_nc, igi->nameid, ik->group.name);
			nc_getname(ipcon_nc, ipn->nameid, ik->group.peer_name);
...
static int ipcon_grp_reg(struct sk_buff *skb, struct genl_info *info)
{
	...
	nameid = nc_add(ipcon_nc, name, GFP_KERNEL);
	if (nameid < 0)
		return nameid;
	...
	do {
		...
		if (!is_publisher(ipn)) {
			ipcon_err("%s: %s is not a publisher.\n",
				__func__, nc_refname(ipcon_nc, ipn->nameid));
			ret = -EPERM;
			break;
		}
...
static int ipcon_grp_reslove(struct sk_buff *skb, struct genl_info *info)
{
	...
	grp_nameid = nc_getid(ipcon_nc, name);
	if (grp_nameid < 0)
		return grp_nameid;
	...
	srv_nameid = nc_getid(ipcon_nc, srvname);
	if (srv_nameid < 0)
		return srv_nameid;
...
static int ipcon_peer_reg(struct sk_buff *skb, struct genl_info *info)
{
	...
	nameid = nc_add(ipcon_nc, name, GFP_ATOMIC);
	if (nameid < 0)
		return nameid;
...
static int ipcon_kernel_init(void)
{
	...
	do {
		...
		kpeer_nameid = nc_add(ipcon_nc, IPCON_NAME, GFP_KERNEL);
		if (kpeer_nameid < 0) {
			ret = kpeer_nameid;
			break;
		}

		kgroup_nameid = nc_add(ipcon_nc, IPCON_KERNEL_GROUP,
				GFP_KERNEL);
		if (kgroup_nameid < 0) {
			ret = kgroup_nameid;
			break;
