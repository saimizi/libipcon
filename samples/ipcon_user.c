...
void ipcon_user_grp_cb(char *peer_name, char *group_name, void *buf,
...
static void ipcon_user_grp_join(char *peer_name, char *group_name, void *data)
{
	ipcon_info("joined %s.%s\n", peer_name, group_name);
}

static void ipcon_user_grp_leave(char *peer_name, char *group_name, void *data)
{
	ipcon_info("left %s.%s\n", peer_name, group_name);
}
...
int main(int argc, char *argv[])
{
	...
	struct async_rcv_ctl arc = { .pgi = pgi,
				     .num = ARRAY_SIZE(pgi),
				     .cb = {
